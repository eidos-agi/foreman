...
`control-hint` is the client discovery path. It starts or reuses a local control-plane API process, records it at `FOREMAN_HOME/control-plane.json`, and returns a live URL. That keeps future Slack, MCP, and web clients from hard-coding ports or parsing process lists.
