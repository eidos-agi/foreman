...
The fields are intentionally generic. There is no engine-specific column, no product-specific column, and no bot-specific column. A job is "run this spec on this repo with this engine, using this contract." Anything beyond that belongs in the spec or in the contract file the spec points at.
...
- **Genericness.** The control plane has nothing to say about what an engine is or what a spec means. That keeps it reusable across product domains. Bot-specific or product-specific behavior belongs in the caller (whatever submits jobs) or in the worker (whatever runs them), not here.
...
- any product-specific or bot-specific schema
