...
def control_serve(args: argparse.Namespace) -> dict[str, Any] | None:
    ...
    state = control_plane_state_payload(host, port, started_at)
    url = state["url"]
    ...
    write_control_plane_state(state)
    if not args.quiet:
    ...
    try:
        ...
        server.server_close()
        remove_control_plane_state(os.getpid())
    return None
...
def load_control_plane_state() -> dict[str, Any] | None:
    if not CONTROL_PLANE_PATH.exists():
        return None
    try:
        return json.loads(CONTROL_PLANE_PATH.read_text(encoding="utf-8"))
    except json.JSONDecodeError:
        return None


def control_plane_state_payload(host: str, port: int, started_at: int) -> dict[str, Any]:
    display_host = "127.0.0.1" if host in {"0.0.0.0", ""} else host
    script = Path(__file__).resolve()
    return {
        "pid": os.getpid(),
        ...
        "url": f"http://{display_host}:{port}",
        "started_at": started_at,
        "state_path": str(CONTROL_PLANE_PATH),
        "log_path": str(CONTROL_PLANE_LOG_PATH),
        "script_path": str(script),
        "script_mtime_ns": script.stat().st_mtime_ns,
    }


def write_control_plane_state(state: dict[str, Any]) -> None:
    STATE_HOME.mkdir(parents=True, exist_ok=True)
    CONTROL_PLANE_PATH.write_text(json.dumps(state, indent=2) + "\n", encoding="utf-8")


def remove_control_plane_state(pid: int) -> None:
    state = load_control_plane_state()
    if state and state.get("pid") == pid:
        CONTROL_PLANE_PATH.unlink(missing_ok=True)
...
def control_plane_health(state: dict[str, Any] | None, timeout: float = 0.5) -> dict[str, Any] | None:
    if not state or not state.get("url"):
        return None
    health = get_json_url(str(state["url"]).rstrip("/") + "/api/health", timeout=timeout)
    if health and health.get("ok"):
...
def stop_control_plane(state: dict[str, Any] | None) -> None:
    if not state or not state.get("pid"):
        return
    try:
        os.kill(int(state["pid"]), 15)
...
def ensure_control_plane(args: argparse.Namespace) -> dict[str, Any]:
    init_db()
    host = getattr(args, "host", "127.0.0.1")
    port = int(getattr(args, "port", DEFAULT_CONTROL_PLANE_PORT))
    existing = load_control_plane_state()
    health = control_plane_health(existing)
    if existing and health and daemon_code_is_current(existing):
        return {
            "status": "running",
            "started": False,
            "control_plane": {**existing, "health": health},
            "url": existing["url"],
        }

    if existing and health and not daemon_code_is_current(existing):
        stop_control_plane(existing)
        time.sleep(0.2)
    CONTROL_PLANE_PATH.unlink(missing_ok=True)
    LOGS_HOME.mkdir(parents=True, exist_ok=True)
    cmd = [
    ...
    log_file = open(CONTROL_PLANE_LOG_PATH, "ab", buffering=0)
    proc = subprocess.Popen(
        cmd,
        stdout=log_file,
        stderr=subprocess.STDOUT,
    ...
    while time.time() < deadline:
        state = load_control_plane_state()
        health = control_plane_health(state)
        if state and health:
            return {
                "status": "running",
                "started": True,
                "control_plane": {**state, "health": health},
                "url": state["url"],
            }
    ...
    return {
        ...
        "state_path": str(CONTROL_PLANE_PATH),
        "log_path": str(CONTROL_PLANE_LOG_PATH),
        "hint": f"tail -80 {CONTROL_PLANE_LOG_PATH}",
    }
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser("control-hint")
    p.add_argument("--host", default="127.0.0.1")
    p.add_argument("--port", type=int, default=DEFAULT_CONTROL_PLANE_PORT, help="port to bind when starting; pass 0 to pick a free port")
    p.set_defaults(func=ensure_control_plane)
