...
LOCAL_FOREMAN = REPO_ROOT / "scripts" / "foreman.py"
FOREMAN = Path(os.environ["FOREMAN_CLI_PATH"]) if os.environ.get("FOREMAN_CLI_PATH") else LOCAL_FOREMAN
...
def foreman_command(args: list[str]) -> list[str]:
    if FOREMAN.exists():
        return [sys.executable, str(FOREMAN), *args]
    return [sys.executable, "-m", "foreman_cli_scripts.foreman", *args]
...
TOOLS = [
    ...
    {
        "name": "foreman_control_hint",
        "description": "Return a live local HTTP URL for the Foreman control-plane API; starts it if needed.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "host": {"type": "string", "default": "127.0.0.1"},
                "port": {"type": "integer", "default": 53640},
            },
        },
    },
    {
...
def call_tool(name: str, arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    if name == "foreman_control_hint":
        cmd = ["control-hint"]
        if arguments.get("host"):
            cmd += ["--host", arguments["host"]]
        if arguments.get("port") is not None:
            cmd += ["--port", str(arguments["port"])]
        return foreman_json(cmd)
